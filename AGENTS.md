...
- Bump `package.json` version on every shipped change (even CSS-only). CSS changes should bump patch version, too. Single branch = 1 version bump, dont bump several versions.
...
1. `npm run dev` → install userscript from `http://127.0.0.1:<port>/__vite-plugin-monkey.install.user.js`
